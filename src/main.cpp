...
unsigned int nStakeTargetSpacing = 30;		// 30 second block time

int64 nChainStartTime = 1427007625;
...
const string strMessageMagic = "Giftcardcoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight > 1000 && nHeight <= 1500)
           nSubsidy = 2000 * COIN;

    else if(nHeight > 1500 && nHeight <= 2000)
           nSubsidy = 1500 * COIN;

    else if(nHeight > 2000 && nHeight <= 2500)
           nSubsidy = 1750 * COIN;

    else if(nHeight > 2500 && nHeight <= 3000)
           nSubsidy = 1000 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_GIFT_PROOF_OF_STAKE;
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 92;
...
static const int64 nTargetTimespan = 60; // 60 Second Blocks  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Giftcardcoin
    bool fStrictPayToScriptHash = true; // Always active in Giftcardcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Giftcardcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeTargetSpacing = 25; // test block spacing is 25 seconds
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Today is the greatest day ever";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1427007625;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2351103;
        ...
        assert(block.hashMerkleRoot == uint256("0x2eff1889d3ef40f9da62a5f48f50be07f5c75eb030015fed435ef5b66a217dde"));
